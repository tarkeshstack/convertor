...
def main():
    ...
    # The app icon has its own branding (a script-glyph mosaic, generated by
    # scripts/gen-icon.js into assets-src/) rather than the live site's own
    # icon, so these two come from there instead of src_dir.
    assets_src = ROOT / "assets-src"
    for asset in ("icon-192.png", "icon-512.png"):
        shutil.copyfile(assets_src / asset, www / asset)
