...
- SpeakEasy's Share button: attaches the spoken translation's mp3 as a real file via the native `@capacitor/share` plugin, not WebView's own Web Share API — that route was tried twice (fetch-on-click, then an eagerly-prefetched fetch) and failed both times, ultimately because the TTS endpoint has no CORS headers and WebView's Web Share Level 2 file support proved unreliable regardless. `@capacitor/http` (bundled with `@capacitor/android`) fetches the mp3 over native networking (not subject to browser CORS), `@capacitor/filesystem` saves it to the cache dir, and `@capacitor/share` hands that file to Android's real share sheet via the FileProvider already declared in AndroidManifest.xml.

All of these plugins (`App`, `StatusBar`, `TextToSpeech`, `SpeechRecognition`, `CapacitorHttp`, `Filesystem`, `Share`) are reached via `window.Capacitor.Plugins.<Name>`, which Capacitor Android auto-injects for every plugin registered in `capacitor.settings.gradle` — no extra `<script>` tags needed. (An earlier version of this bridge tried to load each plugin's own `dist/plugin.js` UMD build via `<script src>`; those reference a `capacitorExports` global that this runtime never defines, so every one of those loads silently threw and left all four plugins non-functional — including the back button and status bar, not just the two speech ones.)
